...
class GoogleDriveBackupManager(private val context: Context) {
    ...
    suspend fun createAndUploadBackup(db: AppDatabase): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            val productReturns = db.productReturnDao().getAllReturns()
            val productReturnItems = db.productReturnDao().getAllReturnItems()
            ...
            val rootJson = JSONObject().apply {
                ...
                productReturns.forEach { ret ->
                    returnsArray.put(JSONObject().apply {
                        put("id", ret.id)
                        put("returnNumber", ret.returnNumber)
                        put("type", ret.type)
                        put("clientId", ret.clientId ?: JSONObject.NULL)
                        put("clientName", ret.clientName ?: "")
                        put("invoiceId", ret.invoiceId ?: JSONObject.NULL)
                        put("invoiceNumber", ret.invoiceNumber ?: "")
                        put("totalAmount", ret.totalAmount)
                        put("settlementType", ret.settlementType)
                        put("date", ret.date)
                        put("notes", ret.notes ?: "")
                    })
                }
                put("productReturns", returnsArray)

                val returnItemsArray = JSONArray()
                productReturnItems.forEach { rItem ->
                    returnItemsArray.put(JSONObject().apply {
                        put("id", rItem.id)
                        put("returnId", rItem.returnId)
                        put("itemId", rItem.itemId)
                        put("itemName", rItem.itemName)
                        put("quantity", rItem.quantity)
                        put("unitPrice", rItem.unitPrice)
                        put("totalPrice", rItem.totalPrice)
                    })
                }
                put("productReturnItems", returnItemsArray)
            }
    ...
    suspend fun restoreFromSpecificFile(db: AppDatabase, targetFile: File): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            // Restore Product Returns & Items
            if (rootObj.has("productReturns")) {
                val arr = rootObj.getJSONArray("productReturns")
                for (i in 0 until arr.length()) {
                    val obj = arr.getJSONObject(i)
                    val ret = ProductReturn(
                        id = obj.optInt("id", 0),
                        returnNumber = obj.getString("returnNumber"),
                        type = obj.optString("type", "CUSTOMER"),
                        clientId = if (obj.has("clientId") && !obj.isNull("clientId")) obj.optInt("clientId") else null,
                        clientName = obj.optString("clientName", ""),
                        invoiceId = if (obj.has("invoiceId") && !obj.isNull("invoiceId")) obj.optInt("invoiceId") else null,
                        invoiceNumber = obj.optString("invoiceNumber", ""),
                        totalAmount = obj.optDouble("totalAmount", 0.0),
                        settlementType = obj.optString("settlementType", "خصم من الرصيد"),
                        date = obj.optLong("date", System.currentTimeMillis()),
                        notes = obj.optString("notes", "")
                    )
                    db.productReturnDao().insertReturn(ret)
                }
            }

            if (rootObj.has("productReturnItems")) {
                val arr = rootObj.getJSONArray("productReturnItems")
                for (i in 0 until arr.length()) {
                    val obj = arr.getJSONObject(i)
                    val rItem = ProductReturnItem(
                        id = obj.optInt("id", 0),
                        returnId = obj.getInt("returnId"),
                    ...
                    db.productReturnDao().insertReturnItem(rItem)
                }
            }
